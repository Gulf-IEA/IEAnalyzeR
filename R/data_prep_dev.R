...
data_prep <-function (df, trends = T, subind = FALSE){
  ...
  ### helper function to create the polygon for the ribbon
  ...
  if (ncol(df_dat)<2.5) {
    ...
    ### superfluous code repeated elsewhere
  ...
  if (trends==T) {
    if(ncol(df_dat)<6){
      ...
      mean_tr<-ifelse(last5_mean>mean+sd, "ptPlus", ifelse(last5_mean<mean-sd, "ptMinus","ptSolid")) #qualify mean trend
      mean_sym<-ifelse(last5_mean>mean+sd, "+", ifelse(last5_mean<mean-sd, "-","●")) #qualify mean trend
      mean_word<-ifelse(last5_mean>mean+sd, "greater", ifelse(last5_mean<mean-sd, "below","within")) #qualify mean trend
      ...
      slope_tr<-ifelse(last5_slope>sd, "arrowUp", ifelse(last5_slope< c(-sd), "arrowDown","arrowRight"))
      slope_sym<-ifelse(last5_slope>sd, "↑", ifelse(last5_slope< c(-sd), "↓","→"))
      slope_word<-ifelse(last5_slope>sd, "an increasing", ifelse(last5_slope< c(-sd), "a decreasing","a stable"))
